 ...
 * type值： （1）：代表使用图片缩放功能，此时，$value1代表缩放后图片的宽度，$value2代表缩放后图片的高度
...
class image
{
    ...
    function __construct ($imageaddress, $types, $value1 = "", $value2 = "", 
            $endaddress)
    {
        $this->types = $types;
    ...
    { // 加图片水印功能
        ...
        // 调用函数计算出水印加载的位置
        $positionarr = $this->position($this->value2, $imagearrs[0], 
                $imagearrs[1]);
        // 加水印
        imagecopy($this->image, $this->imagesources($this->value1), 
                $positionarr[0], $positionarr[1], 0, 0, $imagearrs[0], 
                $imagearrs[1]);
        // 调用输出方法保存
    ...
    { // 图片裁剪功能
        ...
        // 进行裁剪
        imagecopyresampled($newimg, $this->image, 0, 0, $src_x, $src_y, $dst_w, 
                $dst_h, $dst_w, $dst_h);
        // 调用输出方法保存
        $this->output($newimg);
    }
    ...
    private function imagesizex ()
    { // 获得图片宽度
        return imagesx($this->image);
    }

    private function imagesizey ()
    { // 获取图片高度
    ...
    { // 图像缩放功能，返回处理后的图像资源
        ...
        if ($tran >= 0 && $tran < imagecolorstotal($this->image)) {
            ...
            $newcolor = imagecolorallocate($newimg, $tranarr['red'], 
                    $tranarr['green'], $tranarr['blue']);
            imagefill($newimg, 0, 0, $newcolor);
            imagecolortransparent($newimg, $newcolor);
        }
        imagecopyresampled($newimg, $this->image, 0, 0, 0, 0, $this->value1, 
                $this->value2, $this->width, $this->height);
        return $newimg;
    }

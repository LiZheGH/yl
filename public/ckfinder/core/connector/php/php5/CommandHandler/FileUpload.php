...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
	...
	public function cutImage($srcImg, $newSize, $sExtension) {
		$srcWidth = imagesx($this->imagesources($srcImg));
		$srcHeight = imagesy($this->imagesources($srcImg));
		$f = fopen("e:/a.txt", "a");
		fwrite($f, $newSize . "|||||" . $srcWidth . "|||||" . $srcHeight . "|||||" . $sExtension . "\r\n");
		fclose($f);
		$destImg = $srcImg . '_' . $newSize  . '.' . $sExtension;
	
		$image = new image($srcImg, 1,$newSize, $srcHeight * $newSize/$srcWidth,  $destImg);
		if($srcWidth > $newSize) {
			$image->outimage();
		} else {
			copy($srcImg, $destImg);
	...
	public function imagesources ($imgad)
	{ // 获取图片类型并打开图像资源
    ...
    public function sendResponse()
    {
        ...
        //$this->cutImage($sFilePath, 200, $sExtension);
        //$this->cutImage($sFilePath, 260, $sExtension);
        //$this->cutImage($sFilePath, 400, $sExtension);
        //$this->cutImage($sFilePath, 660, $sExtension);

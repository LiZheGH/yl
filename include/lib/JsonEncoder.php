...
class JsonEncoder
{
     ...
     * @return string  The encoded value
     */
    public static function encodeReadable($value, $cycleCheck = false, $options = array())
    {
        $encoder = new self(($cycleCheck) ? true : false, $options);

        return $encoder->_encodeValueReadable($value, 1, true);
    }
     ...
     * @param $value mixed The value to be encoded
     * @param integer $level
     * @param boolean $newLine whether begin with new line
     * @return string Encoded value
     */
    protected function _encodeValueReadable(&$value, $level = 1, $newLine = false)
    {
        if (is_object($value)) {
            return $this->_encodeObject($value);
        } else if (is_array($value)) {
            return $this->_encodeArrayReadable($value, $level, $newLine);
        }
     ...
     * @return string
     */
    protected function _encodeArrayReadable(&$array, $level, $newLine)
    {
        $tmpArray = array();
        ...
        if (!empty($array) && (array_keys($array) !== range(0, count($array) - 1))) {
            ...
            foreach ($array as $key => $value) {
                ...
                $tmpArray[] = "\n" . str_repeat('    ', $level)
                            ...
                            . ':'
                            . $this->_encodeValueReadable($value, $level + 1);
            }
            $result .= implode(',', $tmpArray);
            $result .= "\n" . str_repeat('    ', $level - 1) . '}';
        } else {
            ...
            $length = count($array);
            for ($i = 0; $i < $length; $i++) {
                $tmpArray[] = "\n" . str_repeat('    ', $level) . $this->_encodeValueReadable($array[$i], $level + 1, true);
            }
            $result .= implode(',', $tmpArray);
            $result .= "\n" . str_repeat('    ', $level - 1) . ']';
        }

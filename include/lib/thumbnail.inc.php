...
class Thumbnail {
     ...
     * @param int $height
     */
    public function cropForce($startX,$startY,$width,$height) {
        $oWidth = $this->currentDimensions['width'];
        $oHeight = $this->currentDimensions['height'];

        $revStartX = -$startX;
        $revStartY = -$startY;

        $revStartX = min($width, max(0, $revStartX));
        $revStartY = min($height, max(0, $revStartY));

        // crop
        $endX = $startX + $width;
        $endY = $startY + $height;

        $startX = min($oWidth, max(0, $startX));
        $startY = min($oHeight, max(0, $startY));

        $endX = min($oWidth, max(0, $endX));
        $endY = min($oHeight, max(0, $endY));
